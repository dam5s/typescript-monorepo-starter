...
export type ValidationResult<Input extends ValidationInput, Output> =
    Result<Output, readonly ValidationError<Input>[]>

const ok = <Input extends ValidationInput, Output>(value: Output): ValidationResult<Input, Output> =>
    result.ok(value);

const err = <Input extends ValidationInput, Output>(...errors: readonly ValidationError<Input>[]): ValidationResult<Input, Output> =>
    result.err(errors);
